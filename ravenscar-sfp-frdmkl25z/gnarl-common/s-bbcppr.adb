...
package body System.BB.CPU_Primitives is
   ...
   begin
      ...
      pragma Assert (No_Floating_Point);
      ...
      Asm
        (Template =>
         ...
         --
         --  NOTE: Cortex-M0+ only supports stm/ldm for r0-r7. Also,
         --  for Cortex-M0+ stm/ldm always increment the base register. So,
         --  to make this explicit We use stmia/Ldmia <base reg>!, <reg List>

         "stmia r3!, {r4-r7}"   & NL & -- Save context
         "mov  r4, r8"          & NL &
         "mov  r5, r9"          & NL &
         "mov  r6, r10"         & NL &
         "mov  r7, r11"         & NL &
         ...
         "mov  r2, r3"          & NL &
         "add  r2, r2, #16"     & NL & -- r2 to point to higher registers saved
         "ldmia r2!, {r4-r7}"   & NL & -- Load context and new PSP
         "mov  r8, r4"          & NL &
         "mov  r9, r5"          & NL &
         "mov  r10, r6"         & NL &
         "mov  r11, r7"         & NL &
